...
export class TipoOrdineApiService extends BaseApiService<ITipoOrdine, ITipoOrdineDto>{
  ...
  public addTipoOrdine(tipoOrdine: ITipoOrdine) : Observable<ApiResponse<ITipoOrdine>> {
    const fixedTipoOrdine: ITipoOrdine = this.stringifyDate(tipoOrdine, 'dataTipoOrdine');
  ...
  public getTipoOrdineById(id: string) : Observable<ApiResponse<ITipoOrdine>> {
    return this.getEntityInstanceById(id, this.url).pipe(
      map( (tipoOrdineResult: ApiResponse<ITipoOrdine>) => {return this.convertIntoDate(tipoOrdineResult,'dataTipoOrdine')}))
  }
